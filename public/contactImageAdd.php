...
function ciniki_exhibitions_contactImageAdd(&$ciniki) {
    ...
    $rc = ciniki_exhibitions_checkAccess($ciniki, $args['tnid'], 'ciniki.exhibitions.contactImageAdd', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   

    if( $args['contact_id'] <= 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.exhibitions.7', 'msg'=>'No contact specified'));
    }
    ...
    //
    $strsql = "SELECT id FROM ciniki_exhibition_contact_images "
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.exhibitions.8', 'msg'=>'You already have an image with this name, please choose another name'));
    }
    ...
    return ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.exhibitions.contact_image', $args, 0x07);
}
?>
