...
function ciniki_exhibitions_contactImageGet($ciniki) {
    ...
    $rc = ciniki_exhibitions_checkAccess($ciniki, $args['tnid'], 'ciniki.exhibitions.contactImageGet', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $strsql = "SELECT ciniki_exhibition_contact_images.id, "
        ...
        . "WHERE ciniki_exhibition_contact_images.tnid = '" . ciniki_core_dbQuote($ciniki, $args['tnid']) . "' "
    ...
    if( !isset($rc['images']) ) {
        return array('stat'=>'ok', 'err'=>array('code'=>'ciniki.exhibitions.10', 'msg'=>'Unable to find image'));
    }
